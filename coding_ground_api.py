...
class CodingAIEngine:
        ...
        try:
            ...
            if request.include_docs or request.search_resources:
                docs_context = await self.web_searcher.search_programming_docs(
                    query=request.prompt, language=request.language)
            ...
            response = await self._call_ai_model(request.model,
                                                 enhanced_prompt)
    ...
    async def explain_code(self,
                           request: CodeExplanationRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if request.include_docs:
                ...
                docs_context = await self.web_searcher.search_programming_docs(
                    query=" ".join(concepts), language="general")
        ...
        try:
            ...
            if request.search_docs:
                docs_context = await self.web_searcher.search_error_solutions(
                    error=request.error, code=request.code)

            # Prepare fix prompt
            prompt = self._build_fix_prompt(request.code, request.error,
                                            docs_context)
...
async def generate_project_endpoint(
    request: ProjectRequest, current_user: dict = Depends(verify_token)):
...
"""
        ...
        docs_context = await coding_engine.web_searcher.search_programming_docs(
            query=
            f"{request.language} {request.framework} project structure best practices",
        ...
        response = await coding_engine._call_ai_model("deepseek-r1",
                                                      enhanced_prompt)
...
async def code_review_endpoint(request: CodeReviewRequest,
    ...
    try:
        ...
        docs_context = await coding_engine.web_searcher.search_programming_docs(
            query=
            f"{request.language} code review best practices security performance",
            language=request.language)
    ...
    try:
        ...
        return APIResponse(
            ...
            data={
                ...
                results,
                "sources_searched":
                ["Official docs", "Stack Overflow", "GitHub", "MDN"]
            })
